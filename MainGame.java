...
public class MainGame extends Frame implements Runnable,WindowListener{
   ...
   String[] word=new String[]{"당근","사자","스위스","시루떡","호랑이"};   
   String[] roll=new String[]{"라이어",word[ran.nextInt(word.length)]};
...
public void windowOpened(WindowEvent e) {
   diaLiar=new Dialog(this,"당신의 역할은??");
    laLiar=new Label();
    
    
    diaLiar.add(laLiar);
    diaLiar.addWindowListener(new WindowAdapter() {
   });
    laLiar.setText(roll[ran.nextInt(2)]);
    
    if(roll[ran.nextInt(2)].equals(roll[0])){
       laLiar.setText(roll[0]);
    }else{
       laLiar.setText(roll[1]);
    }
    ...
    diaLiar.setBounds(1000,100,500,400);
    diaLiar.setVisible(true);
